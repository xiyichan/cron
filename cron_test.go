...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", "chenlifeng", func() { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", "chenlifeng", func() { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("* * * * * ?", "chenlifeng", func() { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("@every 2s", "chenlifeng", func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", "chenlifeng", func() {})
	cron.AddFunc("* * * * * ?", "chenlifeng", func() { wg.Done() })
	id1, _ := cron.AddFunc("* * * * * ?", "chenlifeng", func() { t.Fatal() })
	id2, _ := cron.AddFunc("* * * * * ?", "chenlifeng", func() { t.Fatal() })
	cron.AddFunc("0 0 0 31 12 ?", "chenlifeng", func() {})
	cron.AddFunc("* * * * * ?", "chenlifeng", func() { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", "chenlifeng", func() {})
	cron.AddFunc("* * * * * ?", "chenlifeng", func() { wg.Done() })
	cron.Schedule(Every(time.Minute), "chenlifeng", FuncJob(func() {}))
	cron.Schedule(Every(time.Second), "chenlifeng", FuncJob(func() { wg.Done() }))
	cron.Schedule(Every(time.Hour), "chenlifeng", FuncJob(func() {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, "chenlifeng", func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestInvalidJobSpec(t *testing.T) {
	cron := New()
	_, err := cron.AddJob("this will not parse", "chenlifeng", nil)
	if err == nil {
...
func TestJob(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddJob("0 0 0 30 Feb ?", "chenlifeng", testJob{wg, "job0"})
	cron.AddJob("0 0 0 1 1 ?", "chenlifeng", testJob{wg, "job1"})
	job2, _ := cron.AddJob("* * * * * ?", "chenlifeng", testJob{wg, "job2"})
	cron.AddJob("1 0 0 1 1 ?", "chenlifeng", testJob{wg, "job3"})
	cron.Schedule(Every(5*time.Second+5*time.Nanosecond), "chenlifeng", testJob{wg, "job4"})
	job5 := cron.Schedule(Every(5*time.Minute), "chenlifeng", testJob{wg, "job5"})
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), "chenlifeng", FuncJob(func() {}))
	cron.Schedule(Every(time.Second), "chenlifeng", FuncJob(func() {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", "chenlifeng", func() { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), "chenlifeng", FuncJob(func() { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", "chenlifeng", func() {})
		cron.Start()
		cron.AddFunc("* * * * * *", "chenlifeng", func() { time.Sleep(2 * time.Second) })
		cron.AddFunc("* * * * * *", "chenlifeng", func() {})
		time.Sleep(time.Second)
